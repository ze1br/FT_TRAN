...
npm install express socket.io pg cors dotenv bcryptjs jsonwebtoken
...
npm install vite @vitejs/plugin-react react react-dom socket.io-client axios

# Ensure dev script exists
node -e "const pkg = require('./package.json'); pkg.scripts = pkg.scripts || {}; pkg.scripts.dev = 'vite'; require('fs').writeFileSync('package.json', JSON.stringify(pkg, null, 2));"
...
psql -h 127.0.0.1 -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "\dt" 2>/dev/null || echo "(run backend to verify)"
echo ""
